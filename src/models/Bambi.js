...
// Add methods that use the standardized error handling
BambiSchema.statics.findByUsernameWithTimeout = async function(username, timeout = 5000) {
  // Use the timeout wrapper for safe database operations
  return withDatabaseTimeout(
    () => this.findOne({ username }), 
    timeout,
    `Find bambi by username: ${username}`
  );
};

// Add transaction example
BambiSchema.statics.updateWithTransaction = async function(userId, updateData, session) {
