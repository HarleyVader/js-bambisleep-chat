/**
 * Unified Test Runner v3.0 - Intelligent Testing System
 * Simplified, auto-discovering, and performance-optimized
 *
 * Features:
 * - Smart test auto-discovery
...
/**
 * Intelligent test runner with auto-discovery and simplified interface
 */
class BambiSleepTestRunner {
    constructor(options = {}) {
        ...
        this.framework = new UnifiedTestFramework(config);
        this.setupFramework();
    }
    ...
    parseConfig(options) {
        return {
            ...
            verbose: process.env.TEST_VERBOSE === 'true' || options.verbose,
    ...
    setupFramework() {
        ...
        this.framework.beforeAll(async () => {
            console.log('🚀 BambiSleep Chat - Unified Test Suite v3.0');
     ...
     * Run all tests with intelligent filtering and auto-discovery
     */
    async run(filterTags = []) {
        // Use framework's tag configuration for filtering
        const configTags = this.framework.config.tags || [];
        const allTags = [...filterTags, ...configTags];

        if (allTags.length > 0) {
            console.log('🎯 Filter tags:', allTags.join(', '));
        } else {
            console.log('🎯 Running all tests (auto-discovery enabled)');
        }
...
/**
 * Simplified CLI interface with smart argument parsing
 */
async function main() {
    try {
        ...
        const options = {
            ...
            tags: args.filter(arg => arg.startsWith('--tags=')).map(arg => arg.split('=')[1].split(',')).flat(),
            exclude: args.filter(arg => arg.startsWith('--exclude=')).map(arg => arg.split('=')[1].split(',')).flat()
        };

        if (options.help) {
            console.log(`
BambiSleep Chat - Unified Test Framework v3.0

Usage:
  npm test                        Run all tests (auto-discovery)
  npm test -- --tags=critical    Run critical tests only
  npm test -- --watch           Enable watch mode
  npm test -- --verbose         Enable detailed output

Options:
  --tags=tag1,tag2              Run tests with specific tags
  --exclude=tag1,tag2           Exclude tests with specific tags  
  --watch, -w                   Watch files and re-run tests
  --verbose, -v                 Enable verbose output
  --help, -h                    Show this help message

Examples:
  npm test -- --tags=critical,environment
  npm test -- --watch --verbose
  npm run test:watch            # Convenience script for watch mode
            `);
            return;
        }

        // Create and run test runner
        const runner = new BambiSleepTestRunner(options);

        if (options.watch) {
            // Start watch mode - runs indefinitely
            await runner.framework.startWatch();
            await runner.run();

            // Keep process alive for watch mode
            process.on('SIGINT', () => {
                runner.framework.stopWatch();
                console.log('\n👋 Test runner stopped');
                process.exit(0);
            });
        } else {
