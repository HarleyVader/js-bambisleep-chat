/**
 * MCP Tools Integration Test Suite (Unified)
 * Tests all 8 MCP servers for proper activation, authentication, and functionality
 * Merged from mcp-tools.test.js and mcp-tools-simple.test.js
 */
...
class MCPToolsTest {
  ...
  async loadMCPConfig() {
    try {
      ...
      this.log(`Failed to load MCP configuration: ${error.message}`, "error");
      return false;
    }
  }
  ...
  async testServerActivation() {
    ...
    try {
      ...
      for (const [name, config] of Object.entries(
        this.mcpManager.config.mcpServers
      )) {
        try {
  ...
  async testAPIKeys() {
    ...
    try {
      ...
      console.log(
        `✅ API Keys Validation: ${Math.round(
          (validKeys / totalKeys) * 100
        )}% configured`
      );
      return {
        success: validKeys > 0,
        validKeys,
        totalKeys,
        percentage: Math.round((validKeys / totalKeys) * 100),
      };
    } catch (error) {
    ...
    try {
      ...
      try {
        ...
        if (testResult.success) {
          console.log(
            `✅ Authenticated as: ${testResult.username || "brandynette"}`
          );
      ...
      console.log(
        `❌ Hugging Face Authentication test failed: ${error.message}`
      );
      return { success: false, error: error.message };
    }
  }
  ...
  async testStripeAuth() {
    ...
    try {
      ...
      try {
        ...
        if (testResult.success) {
          console.log(
            `✅ Stripe Account: ${testResult.account || "bambisleep.church"}`
          );
  ...
  async testMongoDBConnection() {
    ...
    try {
      ...
      if (testResult.success) {
        console.log(
          `✅ MongoDB: Connected to ${testResult.database || "bambisleep"}`
        );
  ...
  async testGitHubAuth() {
    ...
    try {
      ...
      if (testResult.success) {
        console.log(
          `✅ GitHub: Authenticated as ${testResult.username || "HarleyVader"}`
        );
        return { authenticated: true, username: testResult.username };
      } else {
        throw new Error(testResult.error);
  ...
  async testMCPFunctionality() {
    ...
    for (const { name, test } of functionalityTests) {
      try {
        ...
        if (result.success) {
          ...
          console.log(
            `⚠️  ${name}: ${result.message || "Limited functionality"}`
          );
        }
      } catch (error) {
    ...
    if (passedTests < 3) {
      throw new Error(
        `At least 3/8 servers should be functional, got ${passedTests}/8`
      );
    }
  ...
  async testHuggingFaceFunctionality() {
    ...
    return {
      success: hasKey,
      message: hasKey ? "Model search, datasets available" : "API key needed",
    };
  }
  ...
  async runAllTests() {
    ...
    if (
      report.summary.activeServers === 8 &&
      report.summary.functionalServices >= 6
    ) {
      ...
      console.log(
        "❌ MCP Integration: NEEDS ATTENTION - Configuration required"
      );
    }
      ...
      tests: [
        {
          ...
          details: {
            ...
            overallStatus:
              report.summary.activeServers >= 6
                ? "EXCELLENT"
                : report.summary.activeServers >= 3
                ? "GOOD"
                : "NEEDS_SETUP",
          },
        },
