...
class EnvironmentTester {
    ...
    async testEnvironmentVariables() {
        ...
        this.log(`Optional env vars configured: ${optionalFound}/${optionalEnvVars.length}`, 'info');
        return true;
    }
            ...
            try {
                await new Promise((resolve, reject) => {
                    ...
                    server.listen(portInfo.port, () => {
                        server.close(() => {
                            this.log(`Port ${portInfo.port} (${portInfo.name}) available`, 'pass');
                            resolve();
                        });
                    });
                    ...
                    server.on('error', (err) => {
                        if (err.code === 'EADDRINUSE') {
                            this.log(`Port ${portInfo.port} (${portInfo.name}) in use`, 'warn');
                        } else {
                ...
                this.log(`Port test error for ${portInfo.port}: ${error.message}`, 'fail');
            }
        }
    ...
    async runAllTests() {
        ...
        this.log(`Overall: ${success ? 'ENVIRONMENT OK' : 'ENVIRONMENT ISSUES DETECTED'}`,
            success ? 'pass' : 'fail');
