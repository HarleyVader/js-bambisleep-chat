...
class ResourceTester {
  ...
  async testMemoryUsage() {
    ...
    const growthMB =
      (finalMem.process.heapUsed - initialMem.process.heapUsed) / (1024 * 1024);
    const avgMemUsage =
      memoryData.reduce((sum, m) => sum + m.process.heapUsed, 0) /
      memoryData.length;
    ...
    this.log(
      `Average memory usage: ${avgMemMB.toFixed(2)} MB`,
      avgMemMB < 200 ? "pass" : avgMemMB < 500 ? "warn" : "fail"
    );
    this.log(
      `Peak memory usage: ${peakMemMB.toFixed(2)} MB`,
      peakMemMB < 300 ? "pass" : peakMemMB < 600 ? "warn" : "fail"
    );
    this.log(
      `Memory growth: ${growthMB.toFixed(2)} MB`,
  ...
  async testCPUUsage() {
    ...
    this.log(
      `Average CPU usage: ${avgCpuPercent.toFixed(2)}%`,
  ...
  async testDiskUsage() {
    ...
    try {
      ...
      this.log(
        `Disk write speed: ${writeMBps.toFixed(2)} MB/s`,
        writeMBps > 10 ? "pass" : writeMBps > 1 ? "warn" : "fail"
      );
      this.log(
        `Disk read speed: ${readMBps.toFixed(2)} MB/s`,
  ...
  async testNetworkResources() {
    ...
    try {
      ...
      try {
        ...
        if (successCount > 0) {
          this.log(
            `Network connectivity test: ${successCount}/${requestCount} successful`,
            "pass"
          );
        } else {
          this.log(
            "No server connectivity (expected in resource-only test)",
            "info"
          );
        }
      } catch (error) {
  ...
  async runAllTests() {
    ...
    this.log(
      `Overall: ${success ? "RESOURCE USAGE OK" : "RESOURCE ISSUES DETECTED"}`,
      success ? "pass" : "fail"
    );
  ...
  generateResourceReport() {
    ...
    if (memory.length > 0) {
      const avgMemMB =
        memory.reduce((sum, m) => sum + m.process.heapUsed, 0) /
        memory.length /
        (1024 * 1024);
      const maxMemMB =
        Math.max(...memory.map((m) => m.process.heapUsed)) / (1024 * 1024);
