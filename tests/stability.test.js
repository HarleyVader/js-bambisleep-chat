...
class StabilityTester {
  constructor() {
    ...
    if (this.ciMode) {
      this.log(
        "Running in CI mode - external services will be skipped",
        "info"
      );
    }
  ...
  async testConcurrentConnections() {
    ...
    this.log(
      `Concurrent connections: ${results.connected}/${connectionCount}`,
      results.connected >= connectionCount * 0.8 ? "pass" : "fail"
    );
    this.log(
      `Failed connections: ${results.failed}`,
      results.failed === 0 ? "pass" : "warn"
    );
  ...
  async testMemoryStability() {
    ...
    this.log(
      `Memory growth over ${testDuration / 1000}s: ${growthMB.toFixed(2)} MB`,
  ...
  async runAllTests() {
    ...
    if (this.results.metrics.responseTime) {
      this.log(
        `Avg Response: ${this.results.metrics.responseTime.average.toFixed(
          2
        )}ms`,
        "info"
      );
    ...
    if (this.results.metrics.memoryStability) {
      this.log(
        `Memory Growth: ${this.results.metrics.memoryStability.growthMB.toFixed(
          2
        )}MB`,
        "info"
      );
    ...
    this.log(
      `Overall: ${success ? "STABILITY OK" : "STABILITY ISSUES DETECTED"}`,
      success ? "pass" : "fail"
    );
