...
- **Backend**: Express 5.x + Socket.io + Worker threads (`src/server/server.js`) + Git pull detection + Environment validation  
...
- **Data Flow**: Socket.io ↔ Server ↔ Worker threads (Kokoro TTS only, LM Studio AI)
- **TTS**: Kokoro-FastAPI only (Web Speech API removed in v0.3.0)
...
- **Deployment**: Auto-restart on git pull detection with graceful shutdown
- **MCP Integration**: Framework ready for Model Context Protocol servers (configuration templates in place)
...
src/utils/mcp-manager.js       # MCP server management CLI
```
...
npm run mcp          # MCP server management CLI
```
...
### Git Pull Detection & Auto Deployment (NEW v0.3.0)
```bash
# Automated deployment detection with graceful shutdown
# Server monitors git commit hash every 30 seconds
# On change: Notifies clients → Closes connections → Terminates workers → Exits

# Git pull detection system in server.js:
...
# SystemD service auto-restarts after graceful shutdown
# Prevents stale server instances after deployments
```

### SystemD Service Deployment (Enhanced v0.3.0)
```bash
# Production deployment uses SystemD service with auto-restart
sudo ./install.sh                    # Auto-installs service
npm run validate-service             # Validate service configuration

# Service file: bambisleepchat.service (in project root)
# Note: Service references 'server.js' but actual path is 'src/server/server.js'
# Runs as production user (brandynette), auto-restart on failure + git pull detection
# Logs via journalctl -u bambisleepchat -f
```

### MCP Server Integration (Framework Ready)
```bash
# MCP framework is implemented but servers need configuration:
# Framework supports: GitHub, Hugging Face, Stripe, Clarity, MongoDB, Azure Quantum, Filesystem, ECL

# MCP management commands (once configured):
npm run mcp           # MCP manager CLI - shows configuration status
node src/utils/mcp-manager.js  # Direct CLI access

# To configure MCP servers:
# 1. Create .vscode/mcp-settings.json with server configurations
# 2. Create .env.mcp with API keys and authentication tokens
# 3. Use src/utils/mcp-manager.js as reference for setup
```

### Worker Thread Communication (Enhanced v0.3.0)
```javascript
...
// BREAKING CHANGE v0.3.0: Native fetch API only (Node 18+ required)
// Replaced node-fetch dependency - workers now use native fetch
// Express 5.x upgrade (from 4.x) - improved performance and security
// Kokoro worker uses native fetch for TTS API calls
```
...
### Enhanced Dropdown System (v0.3.0 Major Update)
```javascript
// MAJOR ENHANCEMENT v0.3.0: View Transitions API + Advanced UX
// Consolidated dropdown-utils.js into dropdowns.js (-350 lines net)
// Fixed race conditions and immediate closure bugs

// All dropdowns use unified DropdownManager with enhanced features:
// - View Transitions API integration for 60fps animations
...
// Enhanced component with race condition fix
class ExampleDropdown {
    constructor(dropdownManager) {
        this.dropdownManager = dropdownManager;
        this.componentName = 'example';
        
        // v0.3.0: Centralized click handling prevents duplicate handlers
        // No more immediate closure (<1 second) bugs
    }
...
### Audio Delivery Pattern (v0.3.0 - Kokoro Only)
```javascript
// BREAKING CHANGE v0.3.0: Web Speech API removed, Kokoro-only TTS
...
// Legacy string format support removed in v0.3.0
socket.emit('global-message', {
    display: 'GOOD GIRL',  // What shows in chat
    tts: 'good girl'       // What gets spoken
});

// Worker generates speech via OpenAI-compatible endpoint (Native fetch v0.3.0)
const { KOKORO } = require('../config/env.js');
...
// Public TTS API (v0.3.0) - Legacy exports removed
window.tts.speak(text);     // ✅ Use this
window.tts.enable();        // ✅ Use this
window.tts.disable();       // ✅ Use this
// window.do_tts(text);     // ❌ Removed in v0.3.0
// window.speak(text);      // ❌ Removed in v0.3.0

// Female voices only (v0.3.0): af_bella, af_sky, af_nicole
```
...
git commit -m "test: enhance unified test framework v3.0"
...
- **Unified Testing**: Each test suite supports both unified framework v3.0 and legacy compatibility
- **MCP Integration**: Framework ready for Model Context Protocol servers
...
## System Requirements & Compatibility (v0.3.0)
...
**Tested Versions:**
- Node.js: 18.x, 20.x, 22.19.0
- Express: 5.1.0 (upgraded from 4.x in v0.3.0)

## Breaking Changes (v0.3.0)

**TTS System Modernization:**
- ❌ Web Speech API removed - Kokoro-FastAPI required
- ❌ Legacy TTS exports removed: `window.do_tts()`, `window.speak()`
- ✅ Use modern API: `window.tts.speak()`, `window.tts.enable()`
- ❌ String message format removed
- ✅ Object format required: `{display: "text", tts: "text"}`

**Dependency Updates:**
- ❌ node-fetch removed (6 dependencies eliminated)
- ✅ Native fetch API (Node 18+ required)
- ✅ Express 5.x (performance and security improvements)