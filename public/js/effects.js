    ...
    this.triggers.forEach((trigger) => {
      ...
      processedText = processedText.replace(triggerRegex, (match) => {
        ...
        return `<span class="${additionalClasses}"
                              ...
                              data-category="${
                                triggerInfo.category || "unknown"
                              }"
                              data-safety="${
                                triggerInfo.safetyLevel || "unknown"
                              }"
                              title="${
                                triggerInfo.description ||
                                "Official BambiSleep trigger"
                              }">${match}</span>`;
      });
    });
  ...
  processMessage(text, isAI = false) {
    ...
    setTimeout(() => {
      const triggerElements = document.querySelectorAll(
        ".caps-text[data-trigger]:not(.glitch-processed)"
      );
