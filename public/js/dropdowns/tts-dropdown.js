...
export class TTSDropdown {
  ...
  testVoice() {
    ...
    if (ttsSystem) {
      ttsSystem.speak(
        "Good girl. BambiSleep voices are always feminine and beautiful."
      );
  ...
  syncButtonStateWithTTSSystem(ttsSystem) {
    ...
    const statusIndicator = document.getElementById("tts-status");
    if (statusIndicator) {
      statusIndicator.style.color = targetState === "on" ? "#00ff00" : "#666";
      statusIndicator.textContent = "●";
    }
  ...
  getDropdownContent() {
    return (
      ...
      "<p>Select voices and configure TTS settings</p>" +
      "</div>" +
      '<div class="dropdown-body">' +
      '<div class="dropdown-section">' +
      '<div class="dropdown-section-header">Voice Selection (Max 2)</div>' +
      '<div class="voice-buttons">' +
      this.generateVoiceButtons() +
      "</div>" +
      '<div class="selected-voices-display">' +
      '<p>🔊 Selected: <span id="current-voice-display">' +
      this.getCurrentVoiceDisplay() +
      "</span></p>" +
      "</div>" +
      "</div>" +
      '<div class="dropdown-divider"></div>' +
      '<div class="dropdown-section">' +
      '<div class="dropdown-section-header">Speed Settings</div>' +
      '<button class="dropdown-item" data-action="tts-set-speed-slow">🐌 Slow (0.8x)</button>' +
      '<button class="dropdown-item" data-action="tts-set-speed-normal">⚡ Normal (1.0x)</button>' +
      '<button class="dropdown-item" data-action="tts-set-speed-fast">🌪️ Fast (1.2x)</button>' +
      "</div>" +
      '<div class="dropdown-divider"></div>' +
      '<div class="dropdown-section">' +
      '<div class="dropdown-section-header">Tools</div>' +
      '<button class="dropdown-item" data-action="tts-test-voice">🎵 Test Voice</button>' +
      '<button class="dropdown-item" data-action="tts-clear-cache">🗑️ Clear Cache</button>' +
      '<button class="dropdown-item" data-action="tts-clear-selection">❌ Clear Selection</button>' +
      "</div>" +
      "</div>"
    );
  }
  ...
  generateVoiceOptionsHTML() {
    ...
    return voices
      .map((voice) => {
        const isSelected = this.selectedVoices.includes(voice)
          ? " selected"
          : "";
    ...
    return voices
      .map((voice) => {
        ...
        return (
          ...
          voice +
          '" ' +
          "onclick=\"window.dropdownManager.getComponent('tts').handleVoiceClick(this, '" +
          voice +
          "')\">" +
          voice +
          "</button>"
  ...
  handleVoiceClick(button, voiceName) {
    ...
    if (this.selectedVoices.includes(voiceName)) {
      ...
      this.selectedVoices = this.selectedVoices.filter(
