/**
 * Triggers Dropdown Component for BambiSleep Chat (Refactored to extend BaseDropdown)
 * Handles trigger system dropdown functionality
 */

import { BaseDropdown } from "./base-dropdown.js";

export class TriggersDropdown extends BaseDropdown {
  constructor(dropdownManager) {
    super(dropdownManager, {
      componentName: "triggers",
      buttonId: "toggle-triggers",
      storageKey: "bambi-triggers-state",
      defaultState: {
        isEnabled: false,
        selectedCategories: [],
      },
  ...
  get selectedCategories() {
    return this.getState("selectedCategories") || [];
  }
  ...
  async init() {
    ...
    await this.loadTriggerCategories();
    this.setupEventListeners();
  }

  syncUIWithState() {
    // Update UI to match current state
    this.updateButtonState(this.isEnabled);
  }
  ...
  toggleState(btn) {
    ...
    document.dispatchEvent(
      new CustomEvent("triggerSystemToggle", {
        detail: {
          enabled: newState === "on",
        },
      })
