...
class TriggerSystem {
  ...
  async loadOfficialTriggers() {
    try {
      ...
      console.error(
        "CRITICAL: TriggerSystem failed to load official triggers:",
        error,
      );
  ...
  processAllTriggersWithOverlap(text) {
    ...
    const sortedTriggers = [...this.triggers].sort(
    ...
    sortedTriggers.forEach((trigger) => {
      ...
      processedText = processedText.replace(
        ...
        (match, p1, offset, string) => {
          ...
          const openSpans = (
            beforeMatch.match(/<span[^>]*class="[^"]*trigger[^"]*"[^>]*>/g) ||
            []
          ).length;
          ...
          const alreadyHighlighted = string.includes(
  ...
  dispatchTriggerEvent(triggerName) {
    // Find trigger category from loaded triggers
    let category = "default";

    if (window.chatCore && window.chatCore.allTriggers) {
      const triggerData = window.chatCore.allTriggers.find(
    ...
    // Dispatch custom event for Buttplug integration
    const event = new CustomEvent("trigger-detected", {
      detail: {
