...
class ChatCore {
  ...
  async loadOfficialTriggers() {
    try {
      ...
      if (data.triggers && Array.isArray(data.triggers)) {
        ...
        const primaryTriggers = data.triggers
          .filter((t) => t.category === "primary")
          .slice(0, 2);
        const mentalTriggers = data.triggers
  ...
  async populateTriggerButtons() {
    try {
      ...
      if (this.allTriggers && this.allTriggers.length > 0) {
        ...
        Object.keys(triggersByCategory).forEach((category) => {
          ...
          categoryHeader.textContent = `${category.toUpperCase()} - ${
            this.triggerCategories[category] || "Triggers"
          }`;
      ...
      if (this.triggerContainer) {
        this.triggerContainer.innerHTML =
          '<div class="category-error">Error loading triggers</div>';
      }
    }
  }
  ...
  setupClickOutsideHandling() {
    // REMOVED - All dropdown handling now in dropdowns.js
    // The DropdownManager handles:
    // 1. Button click -> open/close dropdown
    // 2. Click outside -> close all dropdowns
    // 3. Content population
    // See: public/js/dropdowns.js
  }
